...
export async function GET(req: NextRequest) {
  ...
  if (!code) return NextResponse.redirect(new URL("/settings?error=linkedin_denied", req.url));
  try {
    const token = await exchangeCode("linkedin", code);
    db.setToken("linkedin", token);
